# TRACKBACK Propulsion Normal Flow Ground Truth Approval Package v0.3
...
| Initial gear ownership | Gear Software Requirement에 Race start/restart 초기화 포함 | Session / Simulation Initial Condition으로 분리 | Gear Logic 동작과 session setup 책임 분리 |
...
| Speed semantics | `VehicleSpeed=abs(LongitudinalVelocity)` | linear velocity vector magnitude와 forward-axis component로 분리 | 서로 다른 물리량을 독립적으로 관찰 |
| Response precondition | 일반적인 rest 조건 | `abs(LongitudinalVelocity) <= SIM-PROP-ZERO-SPEED-TOLERANCE` | 기존 운동 방향의 즉시 반전을 가정하지 않음 |
...
D↔R 요청은 `abs(LongitudinalVelocity) <= GearDirectionChangeMaxSpeed`일 때만 actual state에 반영할 수 있다. 조건을 만족하지 않으면 GearState는 이전 유효 상태를 유지한다.

### Simulation Initial Condition

아래 값은 Gear Software Requirement가 아니라 TRACKBACK Session / Simulation 초기 조건이다.

```text
InitialGearRequest = D
InitialGearState = D
```

Race start와 restart는 이 초기 조건을 적용한다.
...
| SIG-VEH-SPEED | VehicleSpeed | real/m/s | VehiclePhysics → HUD/VMC/evidence | linear velocity vector magnitude; >=0 | 0 | unavailable + validity metadata |
| SIG-VEH-LONG-VEL | LongitudinalVelocity | real/m/s | VehiclePhysics → Gear Logic/VMC/evidence | vehicle forward-axis velocity component; forward +, reverse - | 0 | unavailable + validity metadata |
...
- **SYSR-PROP-011 — Velocity semantics:** VehicleSpeed shall equal the magnitude of the vehicle linear velocity vector and shall always be non-negative. LongitudinalVelocity shall equal the component of the vehicle linear velocity vector along the vehicle forward axis, shall be positive for forward motion, and shall be negative for reverse motion. VehicleSpeed and LongitudinalVelocity are distinct physical quantities and no equality between VehicleSpeed and `abs(LongitudinalVelocity)` is required.
...
- **SWR-GEAR-001 (Gear Logic contract):** Process GearRequest through the direction-change interlock and reflect a D↔R request in GearState only when `abs(LongitudinalVelocity) <= CAL-GEAR-DIR-CHANGE-MAX-SPEED`; otherwise retain the previous valid GearState. Allocates SYSR-GEAR-001.
...
- **SWR-PHY-002 (VehiclePhysics):** Publish VehicleSpeed as the non-negative magnitude of the vehicle linear velocity vector and publish LongitudinalVelocity as the signed component of that vector along the vehicle forward axis, using consistent m/s units. The software shall calculate and expose these as distinct physical quantities and shall not derive VehicleSpeed solely as `abs(LongitudinalVelocity)`. Allocates SYSR-PROP-011.
...
| TC-PROP-NORMAL-011 | REVISE | SYSR-PROP-007/010/011 | System simulation / complete chain | flat/low-grade dry route, forward clearance, GearState D, valid state, `abs(LongitudinalVelocity) <= SIM-PROP-ZERO-SPEED-TOLERANCE` | apply a non-zero VALID FORWARD propulsion request for `SIM-PROP-RESPONSE-WINDOW` | PropulsionRequest, longitudinal acceleration, LongitudinalVelocity, VehicleSpeed | forward longitudinal acceleration occurs; within the configured response window LongitudinalVelocity > 0; VehicleSpeed is finite and >=0 | near-stationary initial condition에서 forward command가 올바른 축 방향 응답을 만들고 vector speed가 non-negative인지 검증 |
| TC-PROP-NORMAL-012 | REVISE | SYSR-PROP-008/010/011 | System simulation / complete chain | flat/low-grade dry route, reverse clearance, GearState R, valid state, `abs(LongitudinalVelocity) <= SIM-PROP-ZERO-SPEED-TOLERANCE` | apply a non-zero VALID REVERSE propulsion request for `SIM-PROP-RESPONSE-WINDOW` | PropulsionRequest, longitudinal acceleration, LongitudinalVelocity, VehicleSpeed | reverse longitudinal acceleration occurs; within the configured response window LongitudinalVelocity < 0; VehicleSpeed is finite and >=0 | near-stationary initial condition에서 reverse command가 올바른 축 방향 응답을 만들고 vector speed가 non-negative인지 검증 |
...
1. 위 calibration ID들의 실제 값과 표본점은 별도 calibration profile에서 사용자 승인이 필요하다. 특히 TC-011/012의 zero-speed tolerance와 response window가 포함된다. 이는 수치 기반 physics acceptance test를 막지만, 본 architecture/schema와 logical component 구현 계약의 승인을 막지는 않는다.
