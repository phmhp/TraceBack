...
test('Page 1 is one fault report with separated evidence roles and one investigation CTA',()=>{
 ...
 assert.match(page,/OPERATING CONDITION/)
 assert.match(page,/DRIVER \/ SYSTEM INPUT/)
 assert.match(page,/VEHICLE RESPONSE/)
...
test('signal comparison uses actual and expected semantics without fabricating a reference run',()=>{
 ...
 assert.match(view,/ACTUAL vs EXPECTED/)
 assert.match(view,/실제값 · 사건 기록/)
 assert.match(view,/기대값 · 동일 조건의 모델 판정/)
 ...
 assert.match(view,/이 비교를 근거로 기록/)
...
test('interface tracing is graphical, signal-oriented and uses honest component requirement handoff',()=>{
 ...
 const page=readFileSync(new URL('../src/ui/investigation/pages/Page2Tracking.tsx',import.meta.url),'utf8')
 assert.match(view,/FUNCTIONAL INTERFACE TRACE/)
 assert.match(view,/SENDER · 생성/)
 assert.match(view,/RECEIVER · 소비/)
 assert.match(view,/SELECTED SIGNAL PROPAGATION/)
