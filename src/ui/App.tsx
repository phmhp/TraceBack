...
export function App({ scene }: { scene: ReactNode }) {
  ...
  return <div className={`game-shell mode-${screen}`}>
    ...
    {screen === 'xray' && <div className="xray-pip-label" aria-hidden="true"><i />실시간 주행 화면</div>}
    {screen !== 'race' && <div className="build-label">{screen === 'xray' ? 'LIVE X-RAY · RUNTIME MONITOR' : 'PHASE 4.1 · PRESENTATION / ENGINEERING PLACEHOLDERS'}</div>}
  </div>
}
