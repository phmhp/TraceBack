...
function buildMap(view: 'domain' | 'component') {
  ...
  for (const group of groups) {
    ...
    const y = 40 + row * 86
    nodes.push({ id: group, label: group, kind: group === 'Vehicle Dynamics' || group === 'VehiclePhysics' ? 'PLANT / ADAPTER' : view === 'domain' ? 'DOMAIN' : 'COMPONENT', x: 280, y, caption: view === 'domain' ? '탐색용 분류 · 요구사항 아님' : 'Allocated Component 기준' })
    ...
    for (const [column, membersAtLevel] of [system, software].entries()) {
      membersAtLevel.forEach((r, i) => {
        ...
        if (column === 1) {
          nodes.push({ id: `allocation:${r.id}`, label: r.allocatedComponent, kind: r.allocatedComponent === 'VehiclePhysics' ? 'PLANT / ADAPTER ALLOCATION' : 'SW ALLOCATION', x: 1190, y: 40 + (row + i) * 86, caption: r.id })
  ...
  ids.forEach((id, i) => nodes.push({ id, label: id, kind: 'TC', x: 1500, y: 40 + i * 86, caption: tests.find(t => t.id === id)?.testObject ?? '참조 ID · 상세 정의 미연결' }))
  ...
  return { nodes, edges, width: 1750, height: Math.max(row * 86 + 80, ids.length * 86 + 110) }
}

export function RequirementMap({ onComponent, embedded = false, initialRequirement = 'SYSR-PROP-007', onRequirement }: { onComponent: (id: XRayNodeId) => void; embedded?: boolean; initialRequirement?: string; onRequirement?: (id: string) => void }) {
  const [view, setView] = useState<'domain' | 'component'>('domain')
  const [selected, setSelected] = useState(initialRequirement)
  const [zoom, setZoom] = useState(embedded ? .85 : .65)
  ...
  const select = (n: Node) => { setSelected(n.id); const r = requirements.find(r => r.id === n.id || `allocation:${r.id}` === n.id); if (r) { onComponent(r.allocatedComponent); if (n.id === r.id) onRequirement?.(r.id) } }
  ...
  useLayoutEffect(() => {
    if (!embedded) return
    const n = graph.nodes.find(n => n.id === initialRequirement)
    if (n && viewport.current) viewport.current.scrollTo({left: Math.max(0,(n.x - 220)*.85),top:Math.max(0,(n.y-60)*.85)})
  }, [embedded, initialRequirement, graph])
  return <div className={`requirement-map ${embedded ? 'embedded' : ''}`}>
    <div className="req-map-toolbar"><div aria-label="관점"><button aria-pressed={view === 'domain'} onClick={() => setView('domain')}>도메인별</button><button aria-pressed={view === 'component'} onClick={() => setView('component')}>SW 구성요소별</button></div><span>빈 공간 드래그 · 노드 선택</span><div><button aria-label="축소" onClick={() => setZoom(z => Math.max(.3, z - .1))}>−</button><output>{Math.round(zoom * 100)}%</output><button aria-label="확대" onClick={() => setZoom(z => Math.min(1.5, z + .1))}>+</button><button onClick={locate}>선택 위치</button><button onClick={() => { setZoom(.5); viewport.current?.scrollTo(0, 0) }}>처음 위치</button></div></div>
